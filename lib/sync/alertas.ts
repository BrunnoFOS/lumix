...
async function fetchFromWebhook(url: string): Promise<RawAlerta[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[sync-alertas] webhook ${url} retornou ${res.status}`);
      return [];
    }
...
async function syncProvider(
  ...
  const raw = await fetchFromWebhook(webhookUrl);

  if (raw.length > 0) {
    const payload = raw.map((a) => normalizeAlerta(a, provider));
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data, error } = await (supabase.rpc as any)("sync_alertas_cache", {
      p_provider: provider,
      payload,
    });
  ...
  return error ? { error: error.message } : { cleared: true };
}

export async function syncAlertasCache() {
  try {
    const supabase = getServiceClient();

    const [solis, sungrow] = await Promise.all([
      syncProvider(supabase, "solis", WEBHOOK_SOLIS),
      syncProvider(supabase, "sungrow", WEBHOOK_SUNGROW),
    ]);

    console.log("[sync-alertas] sync concluido:", { solis, sungrow });
  } catch (err) {
    console.error(
      "[sync-alertas] erro geral:",
      err instanceof Error ? err.message : err
    );
  }
}
